#!/bin/bash
# ============================================================================
# RISCOF RV32 D — Compile + Run + Coverage
# ============================================================================
# Compiles RV32 D .S tests, runs through gcov-Spike, collects coverage.
# NOTE: model_test.h was patched to use ALIGNMENT=3 when FLEN=64
#
# Usage: bash run_riscof_rv32_D.sh
# ============================================================================
...
ELF_BASE="/tmp/riscof_rv32_d_elf"
OUTPUT_BASE="/tmp/riscof_rv32_d_results"
...
echo "============================================"
echo "  RISCOF RV32 D Coverage Collection"
echo "============================================"
...
# ========================================================================
echo "============================================"
echo "  Phase 1: Compile RV32D ELF files"
echo "============================================"
...
for src in $file_list; do
    ...
    if docker exec "$CONTAINER" bash -c "
        $GCC \
            ...
            -mabi=$MABI \
            '$src' -o '$elf_path' \
            -DTEST_CASE_1=True -DXLEN=32 -DFLEN=64 2>&1
    " 2>/dev/null; then
        compiled=$((compiled + 1))
    else
        failed=$((failed + 1))
...
echo ""
echo "============================================"
echo "  Phase 2: Run RV32D Tests & Collect Coverage"
echo "============================================"
...
OUTPUT_INFO="$OUTPUT_BASE/riscof_rv32_D_coverage.info"
...
docker cp "$CONTAINER:$OUTPUT_INFO" "$LOCAL_OUT/riscof_rv32_D_coverage.info" 2>/dev/null
echo "  Saved: riscof_rv32_D_coverage.info"

echo ""
echo "============================================"
echo "  DONE — RV32D"
echo "============================================"
ls -la "$LOCAL_OUT/riscof_rv32_D_coverage.info" 2>/dev/null | awk '{printf "  %s (%s)\n", $NF, $5}'
